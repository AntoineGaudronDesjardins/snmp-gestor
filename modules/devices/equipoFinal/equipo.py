    ...
    def initTraps(self):
        ...
        def initDismanTable(tableName):
            for entry in switchTrapConfig[tableName]:
                ...
                if existingRow:
                    ...
                    if identical:
                        ...
                        if len(entryStatusColumn) == 1:
                            ...
                            self.snmpEngine.setTableRow('DISMAN-EVENT-MIB', index.values(), (entryStatusColumn[0], 'destroy'), auth="bootstrap")
                ...
                newRow = self.snmpEngine.setTableRow('DISMAN-EVENT-MIB', index.values(), *args, auth="bootstrap")
                if not newRow: 
        ...
        for entry in switchTrapConfig['mteTriggerTable']:
            self.snmpEngine.set('active', 'DISMAN-EVENT-MIB', 'mteTriggerEntryStatus', *entry["index"].values(), auth="bootstrap")
        
        for entry in switchTrapConfig['mteEventTable']:
            self.snmpEngine.set('active', 'DISMAN-EVENT-MIB', 'mteEventEntryStatus', *entry["index"].values(), auth="bootstrap")
